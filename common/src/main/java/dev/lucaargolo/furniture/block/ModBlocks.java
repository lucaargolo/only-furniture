...
public class ModBlocks {
    ...
    public static final Map<WoodType, Supplier<FancyFenceBlock.Hedge>> HEDGE_MAP = registerForHedge("hedge", WoodBlock::getLeaves, WoodBlock::getLeavesColor, FancyFenceBlock.Hedge::new, 4f, ModBlockTags.CONNECTING_HEDGES, BlockTags.MINEABLE_WITH_AXE);
    ...
    private static <T extends Block> Map<WoodType, Supplier<T>> registerForHedge(String path, Function<WoodType, Optional<Block>> baseGetter, Function<WoodType, TintColor.Block> blockColorGetter, QuadFunction<Block, TagKey<Block>, WoodType, Float, T> furnitureConstructor, float size, TagKey<?>... tags) {
        return registerForWoodType(path, baseGetter, blockColorGetter, (block, wood, shapes) -> furnitureConstructor.apply(block, tags[0].cast(Registries.BLOCK).orElseThrow(), wood, size), ModBlockShapes.EMPTY, tags);
    }
